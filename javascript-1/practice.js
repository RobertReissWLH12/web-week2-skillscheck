...
let total = myNumbers.reduce((total, current) => {return total += current});
...
let myNumbersIndex = []
myNumbersIndex.forEach(function(el) {
	alert(el[i].push)
})
...
let enemies = people.filter((relationship) => relationship.friend === false)
...
totallyAwesome.reduce((acc, person) => {
